...
int main() {
    try {
        ...
        for (int i = 0; i < 2000; ++i) a.agregarEstado("extra" + std::to_string(i));
        verificar(q0 == a.buscarEstado("q0"));
        verificar(q0->transiciones()[0].destinos()[0] == q0);
        auto copia = a.clonar();
        verificar(copia.buscarEstado("q0") != q0);
        verificar(copia.buscarEstado("q0")->transiciones()[0].destinos()[0] == copia.buscarEstado("q0"));
        auto movido = std::move(copia);
        movido.validar(); verificar(movido.acepta({"a"}));
        auto d = convertir.convertir(a);
        auto m = minimizar.minimizar(a);
        verificar(d.esDeterministico() && m.estados().size() == 2);
        verificar(equivalencia.comparar(a, m).equivalentes);
        Automata e;
        e.agregarEstado("s"); e.agregarEstado("f", true); e.establecerInicial("s");
        e.agregarTransicion("s", "", {"f"}); e.agregarTransicion("f", "", {"s"});
        ...
        Automata vacio;
        vacio.agregarEstado("s"); vacio.establecerInicial("s");
        verificar(minimizar.minimizar(vacio).estados().size() == 1);
        ...
        verificar(leido.inicial()->transiciones()[0].destinos().empty());
        ...
        debeFallar([&]{ a.acepta({""}); });
        {
            std::ofstream out("test_invalido.automata"); out << "AUTOMATA_V1\n-1\n";
        }
        debeFallar([&]{ archivos.cargar("test_invalido.automata"); });
        std::remove("test_invalido.automata");
        // Comprobar por enumeracion, independientemente de la equivalencia,
        // las palabras de longitud 0..5 en 80 AFN con ciclos epsilon aleatorios.
        std::mt19937 azar(42);
        for (int caso = 0; caso < 80; ++caso) {
            Automata nfa;
            for (int i = 0; i < 5; ++i) nfa.agregarEstado(std::to_string(i), azar() % 2 != 0);
            nfa.establecerInicial("0");
            for (int i = 0; i < 5; ++i) for (const auto& s : {"", "a", "b"}) {
                std::vector<std::string> destinos;
                for (int j = 0; j < 5; ++j) if (azar() % 5 == 0) destinos.push_back(std::to_string(j));
        ...
        std::cout << "OK: punteros, copia, movimiento, AFN, epsilon, AFD, minimizacion, equivalencia, archivos y UI.\n";
        return 0;
